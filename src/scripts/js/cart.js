...
$(function() {
    ...
    shopList.forEach(item=>{
        ...
        <div class="good-item">
            ...
            <div class="item-msg" name="name1">${item.name}</div>
        </div>
        </li>
        <li class="yui3-u-1-8">
        ${item.name}
        ...
        <li class="yui3-u-1-8">
            <span class="price">${Number((item.price).slice(1)).toFixed(2)}</span>
        </li>
        <li class="yui3-u-1-8">
            <div class="clearfix">
                <a href="javascript:;" class="increment mins">-</a>
                <input autocomplete="off" data="${item.name}" name="num1" type="text" value="${item.num}" minnum="1" class="itxt">
                <a href="javascript:;" class="increment plus">+</a>
            </div>
            <div class="youhuo">有货</div>
        </li>
        <li class="yui3-u-1-8">
            <span class="sum">${((item.price).slice(1)*item.num).toFixed(2)}</span>
        </li>
        <li class="yui3-u-1-8">
            <div class="del1" data="${item.name}">
                <a href="javascript:;">删除</a>
            </div>
            <div>移到我的关注</div>
        </li>
    ...
    function me_sum() {
        ...
        if(sum>99){
            ...
            $(".yunfei span").html(`还差￥${(99-sum).toFixed(2)}免运费`)
        }
    ...
    $(".goods-list .plus").click(function() {
        ...
        // console.log($(this).siblings(".itxt").attr("data"))
        shopList.map(item=>{
            if(item.name==$(this).siblings(".itxt").attr("data"))
                item.num++;
        })
        // console.log(shopList)
    ...
    $(".del1 a").click(function() {
        ...
        shopList = shopList.filter(item=>{
            return item.name!=$(this).parent().attr("data")
        })
        // console.log(shopList)
        localStorage.setItem("shopList" , JSON.stringify(shopList));
        me_sum();
